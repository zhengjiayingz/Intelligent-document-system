...
> 版本：v1.39  
> 日期：2026-07-08（S13～S19 更新见修订记录；**本批确认 F-30+搜图+查重：2026-07-20**；**F-40 整理文件夹远期：2026-07-20**；**F-30 ✅ 2026-07-21**；**F-38 ✅ 2026-07-21**；**F-39 ❌ 砍掉 2026-07-21**；**F-35 ✅ 完全重复查重 2026-07-21**；**F-24 设计：2026-07-21**；**里程碑修订：2026-07-22～23**；**F-41 文稿纠偏（不挡里程碑）：2026-07-23**；**F-42 AI 助手只读 FC MVP ✅：2026-07-23**；**ai/ 目录重构 ✅：2026-07-23**；**F-24 ⏸ 押后：2026-07-24**；**F-25 视频 ✅ + ⚑：2026-07-25**；**F-25 语义章节摘要（改需求）：2026-07-25**）  
> 状态：**§1.5 ⚑ 已达成**；**当前改需求：** F-25 **音视频语义章节摘要 + 点击跳播**（待设计/实现审核）；**F-24 ⏸**；其后：F-31～ / F-41 / F-18 / 插件 / F-40 / F-20）  
...
> F-24 设计（⏸ 押后）：[2026-07-21-F24-MCP暴露网盘-设计.md](./2026-07-21-F24-MCP暴露网盘-设计.md)  
> F-25 音频 ASR 设计：[2026-07-22-F25-音频转文字-ASR-设计.md](./2026-07-22-F25-音频转文字-ASR-设计.md)  
> F-25 视频抽音轨（✅）：[2026-07-23-F25-视频抽音轨与总结-实现方案.md](./2026-07-23-F25-视频抽音轨与总结-实现方案.md)  
> **F-25 语义章节摘要（改需求，待审）：** [设计](./2026-07-25-F25-音视频语义章节摘要-设计.md) · [实现方案](./2026-07-25-F25-音视频语义章节摘要-实现方案.md)  
> **运行时优化清单（断点续建 / 索引与检索加速）：** [2026-07-25-运行时优化清单.md](./2026-07-25-运行时优化清单.md)  
...
| **4′** | 视频抽音轨 / 视频总结 | **F-25（视频）** | ✅ **已完成**（2026-07-24 手测：文稿/字幕可演示；方案见 [视频实现方案](./2026-07-23-F25-视频抽音轨与总结-实现方案.md)） |
| **…** | MCP 暴露网盘 | **F-24** | ⏸ **押后**（2026-07-24：实用价值偏低；设计保留备查） |
| **⚑** | 阶段成果 | — | ✅ **已打标**（2026-07-25）：音频 ASR + TTS + 视频抽轨/文稿闭环 |

**相对 2026-07-21 原「四段」的变更说明**

- 原计划：先视频（段 2）→ MCP（段 3）→ 音频（段 4）。  
- **实际执行：** **先做音频 ASR**，再建共享 ASR 层；**紧接着做 TTS**；**视频抽音轨延后**到 `ai/` 重构之后。  
- MCP（F-24）设计保留，**2026-07-24 起明确押后、不进里程碑排期**。
...
│ L0″ 语音：ASR 音频/视频转写 ✅ / TTS 文字转语音 ✅（划词+独立页） │
└─────────────────────────────────────────────────────────────┘
```
...
# 视频时长上限（F-25；默认 3600=1 小时）
# AI_MAX_VIDEO_DURATION_SEC=3600
```
...
| **优先级** | P2 |
| **价值** | 5 |
| **难度** | 4（转写/抽轨已交付；语义章节为增量） |
| **代号** | S17b |
| **依赖** | F-01、F-03（共用 `DocumentSummary` + `generateObject`）、媒体时长（ffprobe） |
| **工期** | 音频 ASR ✅；视频抽轨/文稿 ✅；**语义章节摘要 🔜 约 2～4 天** |
| **交付分期** | 见下表 |
| **音频转写** | ✅ [ASR 设计](./2026-07-22-F25-音频转文字-ASR-设计.md) / [实现方案](./2026-07-21-F25-音频转文字-实现方案.md) |
| **视频抽音轨** | ✅ [视频实现方案](./2026-07-23-F25-视频抽音轨与总结-实现方案.md) |
| **语义章节摘要** | ✅ 代码已落地（2026-07-25）；[设计](./2026-07-25-F25-音视频语义章节摘要-设计.md) · [实现方案](./2026-07-25-F25-音视频语义章节摘要-实现方案.md) |
| **TTS（关联）** | ✅ [TTS 实现方案](./2026-07-22-TTS-文字转语音-实现方案.md) |
| **状态** | 转写/抽轨/播放器文稿 ✅；**语义章节跳播 代码已落地，待手测** |

#### 需求描述（现行）

用户对网盘内 **视频/音频** 触发「建立索引」后：

1. **已交付：** 抽音轨（视频）→ ASR 转写 → 文稿面板；可有全书级一句话/概览类摘要（沿用文本摘要链路）。
2. **本次改需求（待实现）：** 在转写全文基础上，由 **AI 按大意自动分段**（段数不固定），每段给出 **标题 + 短摘要 + 可跳转时间**；用户在播放器「摘要」Tab **点击某段 → 跳到对应时刻播放**。体验对齐文本文件「分层摘要」心智，但输出是「带时间的语义章节」，不是固定切 N 段。

MVP **仅网盘上传文件**；不含 YouTube/B 站外链。
...
|------|------|------|
| 音频 ASR | 预处理 + 转写 + 文稿 | ✅ |
| 视频抽轨 | ffmpeg → 复用 ASR → 文稿/VTT/SRT | ✅ |
| **语义章节摘要 + 跳播** | LLM 按大意分段；段数自定；点摘要 seek | ✅ 代码已落地（待手测） |
| 文稿纠偏 F-41 | 自动纠偏 + 手改 | 不挡本需求 |
| 句级/字级字幕卡点 | 依赖句级 ASR 时间轴 | ❌ **本期不做**（硅基 SenseVoice 常无可靠 segments） |
| 外链 / 本地 Whisper | — | 二期 |
...
| 规则 | 说明 |
|------|------|
| **分段依据** | **按大意/主题变化**，由 AI 判断；**禁止**写死「每个文件一律 5 段」 |
| **段数** | 内容有几段主题就几段；允许约 **1～12**（过短可 1 段总览；过碎合并；过粗可再拆，由 Prompt + 校验约束） |
| **每段展示** | 标题、短摘要、起止时间（或起始时间）；点击跳播 |
| **全书层** | 保留一句话 / 总览（与文本摘要类似） |
| **时间来源（MVP）** | **不依赖**句级 ASR；用「章节在全文中的位置比例 × 媒体总时长」估算 startMs/endMs（有可靠句级时间轴时可二期升级） |
| **入口** | 复用现有「建立索引」；索引完成后摘要 Tab 展示可点章节列表 |

#### 验收标准（语义章节增量）

- 音/视频索引 `ready` 后，摘要 Tab 出现 **≥1** 条语义章节（常见为多段，段数随内容变）
- 每条含标题、摘要、可显示时间；**点击后播放器跳到对应秒数并播放**
- **不同样例段数可以不同**（不强制恒为 5）
- 二次打开读库，不重复跑 ASR/章节 LLM（除非 force 重建）
- ASR 仍为「一坨」全文时，章节跳转**仍可用**
- 普通文本文档摘要行为无回归

#### 不做（本期）

- 歌词级 / 电视剧逐句字幕卡点；换句级 ASR 模型
- 固定均分 N 段（含写死 5 段）
- 外链视频、分享页展示媒体摘要（可后续）
- F-41 纠偏
...
- 在现有 ASR「全文一坨」约束下，仍提供可演示的 **摘要导航听播**
- 与 F-03 文本摘要叙事统一：**LLM 结构化理解 + 可点击定位**
- 简历可讲：媒体转写 → 语义分段 → 时间映射 → 播放器联动（无需新框架）

**关联详案：** 设计 / 实现方案见文首链接（2026-07-25）。历史技术方案草案（抽轨、枚举 video_overview 等）以已落地代码与上述改需求为准，不再作为语义章节的实现约束。
...
> **F-24** 设计已确认，**2026-07-24 起 ⏸ 押后**（不进里程碑排期）。  
...
| F-24 | MCP 暴露网盘 | 2 | 3 | **⏸ 押后** | 本机 MCP 只读；[设计](./2026-07-21-F24-MCP暴露网盘-设计.md)；实用价值偏低 |
...
| **与里程碑关系** | **明确不插入 §1.5 主线**；不阻塞已完成的 ai/ 重构、视频抽音轨、⚑ 打标 |
...
> **下一批（🔜）：** **F-25 语义章节摘要 + 跳播**（v1.39 改需求）；其后视觉线 F-31→…；**§1.5 ⚑ 已达成**  
> **里程碑主线（修订后）：** 视觉收口 ✅ → **音频 ASR ✅** → **TTS ✅** → ai/ 重构 ✅ → **视频 ✅**（**F-24 MCP ⏸ 押后**）  
> **其后视觉线：** F-31 → F-32 → F-33 → F-36 → F-37  
> **延后：** F-18；插件押后  
> **远期不排期：** F-40（AI 整理 B→C）、F-20（全量 Agent）、F-21 / F-23、**F-41（文稿纠偏/手改，不挡里程碑）**  
> **已完成（本批及后续）：** F-30、**F-38**、**F-35（完全重复查重）**、**F-25 音频 ASR + 视频**、**TTS MVP**、**F-42 AI 助手只读 MVP**、**ai/ 目录重构**  
...
| 16 | F-25 | **视频/音频总结** | P2 | 5 | 4 | ✅ 转写/抽轨；🔜 语义章节 | S17b；见 v1.39 改需求 |
...
| 37 | F-24 | MCP 暴露网盘 | — | 2 | 3 | **⏸ 押后** | [设计](./2026-07-21-F24-MCP暴露网盘-设计.md)；2026-07-24 押后 |
...
**下一批（🔜）：** **F-25 语义章节摘要 + 跳播**；其后 F-31 → F-32 → F-33 → F-36 → F-37  
**押后 / 不排期：** **F-24 MCP**；F-20 全量 Agent、**F-40 AI 整理文件夹（B→C）**、F-21、F-23、F-41  
**已完成（含本批）：** F-05、F-06、F-07、F-11、F-08、F-26、F-27、F-29、**F-30**、**F-38**、**F-35（完全重复查重）**、**F-25 音频 ASR + 视频**、**TTS MVP**、**F-42**、**ai/ 重构**、**§1.5 ⚑**  
...
| — | **2026-07-24** | **F-25 视频抽音轨** | ✅ 手测文稿/播放器面板通过；§1.5 ⚑ |
...
| [2026-07-22-AI目录结构重构-里程碑.md](./2026-07-22-AI目录结构重构-里程碑.md) | ai/ 目录重构 ✅；其后 F-25 视频已交付 |
| [2026-07-23-AI目录重构-执行规范.md](./2026-07-23-AI目录重构-执行规范.md) | 重构权威规则与进度（C1～A8 ✅） |
| [2026-07-23-F25-视频抽音轨与总结-实现方案.md](./2026-07-23-F25-视频抽音轨与总结-实现方案.md) | F-25 视频 ✅（手测 2026-07-24） |

**下一步行动：** **F-25 语义章节摘要 + 点击跳播**（设计/实现方案待审，见 2026-07-25 文档）。§1.5 ⚑ 已达成。其后可选：视觉线 F-31～、**F-41**、开放前 **F-18**；**F-24 MCP ⏸**。
...
| v1.37 | 2026-07-24 | **F-24 MCP ⏸ 押后**：实用价值偏低，移出里程碑排期；设计文档保留备查；下一步仍为视频抽音轨 |
| v1.38 | 2026-07-25 | **F-25 视频 ✅ + §1.5 ⚑**：代码与手测（2026-07-24）确认抽音轨/文稿/播放器面板已交付；同步总表/排期/依赖图；其后为 F-31～ 等 |
| v1.39 | 2026-07-25 | **F-25 改需求：语义章节摘要 + 点击跳播**：AI 按大意分段（段数不固定）；时间用文稿位置×时长估算；不做句级字幕/固定 N 段；增补设计与实现方案链接 |
